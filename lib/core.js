...
function src_themes() {
    return gulp.src("wikismith_themes/**/gulpfile.js")
        .pipe(decorateModule())
}
...
function watch_themes_all() {
    return watch({glob:'wikismith_themes/**/*', emitOnGlob: false})
        .pipe(get_module_gulpfile())
        .pipe(throttle_duplicates('path',1000))
        .pipe(decorateModule())	
}

function watch_themes() {
    return watch({glob:'wikismith_themes/*/*.*', emitOnGlob: false})
        .pipe(get_module_gulpfile())
        .pipe(throttle_duplicates('path',1000))
        .pipe(decorateModule())
...
function pageContent(file) {
    try {
        ...
        else
        {
            ...
            var modulePath = path.join(process.cwd(), 'wikismith_themes', module_name, 'index.js');
